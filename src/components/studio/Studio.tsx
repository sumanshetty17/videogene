  ...
  return (
    <div className="flex flex-col gap-3">
      <textarea
        ...
        rows={12}
        placeholder="Paste any topic: a battery, the heart, photosynthesis, a history chapter, a product spec…"
      ...
      <Button onClick={onGenerate} disabled={generating || !localSource.trim()}>
        ...
        {generating ? "Building video…" : "Generate video with audio"}
      </Button>
      <button
        type="button"
        className="text-left text-xs text-muted underline-offset-2 hover:text-fg hover:underline"
        onClick={() => setLocalSource(SAMPLE_SOURCE)}
      >
        Or try the motor sample
      </button>
    </div>
  );
}
          ...
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-bg/70 p-4 backdrop-blur-sm sm:p-8">
            <div className="w-full max-w-xl rounded-[var(--radius-lg)] border border-border bg-surface p-5 shadow-lg sm:p-7">
              ...
              <p className="mt-2 text-sm leading-relaxed text-muted">
                Paste any subject or upload a text file. We build a narrated 3D
                video that explains what you entered — not a fixed motor demo.
              </p>
              <div className="mt-5">
                <ComposeForm
        ...
        <aside className="flex w-full flex-col gap-4 overflow-y-auto border-t border-border bg-surface p-4 sm:p-5 lg:w-[400px] lg:border-t-0 lg:border-l">
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-muted">
              Named from your text. Each one lights up when the voice says it.
            </p>
          </div>
          {focused ? (
