...
export function BoneCharacter({
  ...
  useFrame((state, delta) => {
    ...
    const walk = action === "walk" || action === "flee" || action === "charge";
    const talk = action === "talk";
    const point = action === "point";
    const look = action === "look" || action === "react";
    const fight = action === "fight";
    const cycle = walk ? t * (action === "flee" ? 8.2 : 6.2) : fight ? t * 9 : t * 1.7;
    ...
    if (rArm.current) {
      ...
      rArm.current.rotation.x = fight
        ? -1.4 + Math.sin(cycle) * 0.9
        : point
          ? -1.15
          : talk
    ...
    if (lArm.current) {
      lArm.current.rotation.x = fight
        ? -1.1 + Math.cos(cycle) * 0.7
        : walk
          ? -swing * 0.55
          : talk
            ? -0.25
  ...
  const pants = horns ? "#1a0c08" : "#1c222c";

  return (
    <group position={position} ref={root} scale={scale}>
      <group ref={hips}>
