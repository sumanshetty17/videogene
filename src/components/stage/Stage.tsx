...
function SceneContents() {
  ...
  const parts = lesson.parts?.length ? lesson.parts : MOTOR_PARTS;
  const isMotor = lesson.device === "motor";
  const onDevice = scene.world === "device";
  const explode = onDevice
    ? Math.max(scene.motorExplode, focus ? 0.9 : 0) * Math.min(1, 0.4 + sceneProgress)
    : scene.motorExplode;
  const workshop = scene.world === "workshop";
  ...
  }, [focus, focusIndex, explode, isMotor]);
